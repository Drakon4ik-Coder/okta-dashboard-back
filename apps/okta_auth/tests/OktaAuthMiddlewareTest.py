...
from okta_auth.middleware import OktaAuthMiddleware
from okta_auth.views import login_view, callback_view, logout_view
...
class OktaAuthMiddlewareTest(TestCase):
    ...
    @patch('okta_auth.middleware.requests.get')
    ...
    def test_invalid_access_token(self, mock_get):
        ...
        # Process request with middleware
        with patch('okta_auth.middleware.redirect') as mock_redirect:
...
class OktaViewsTest(TestCase):
    ...
    @patch('okta_auth.views.settings')
    ...
    @patch('okta_auth.views.requests.post')
    def test_callback_view(self, mock_post):
        ...
        with patch('okta_auth.views.get_user_info') as mock_user_info:
