...
def ratelimit_view(rate, key='ip'):
    """
    Custom decorator to apply rate limiting and render a custom template
    ...
    def decorator(view_func):
        ...
        def wrapped_view(request, *args, **kwargs):
            if getattr(request, 'limited', False):
                return render(request, 'traffic_analysis/ratelimit.html', status=429)
