...
class ComputeAvgLoginTimeTest(TestCase):
    @patch('login_tracking.utils.DatabaseService')
    def test_reverse_order_pairing(self, mock_db_service):
        base = datetime(2025,5,1,12,0,0, tzinfo=pytz.UTC)
        logs = [
            # session.start then authorize.code ⇒ should compute +5 s
            {"eventType":"user.session.start", "actor":{"id":"u"}, "_published_date":base},
            {"eventType":"app.oauth2.authorize.code", "actor":{"id":"u"}, "_published_date":base+timedelta(seconds=5)},
        ]
        mock_db = MagicMock()
