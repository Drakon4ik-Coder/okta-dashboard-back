...
from traffic_analysis.models import OktaEvent, OktaMetrics, ForensicEvent
...
class EventService:
    ...
    def get_event_types() -> List[str]:
        ...
        """
        # Use MongoDB distinct operation to get unique values efficiently
        return OktaEvent.objects().distinct('event_type')
    ...
    def get_severity_counts(start_time: datetime, end_time: datetime) -> Dict[str, int]:
        ...
        result = OktaEvent.objects.aggregate(*pipeline)
...
class EventAnalysisService:
    ...
    def get_top_ip_addresses(
        ...
        pipeline = [
            ...
            {"$match": {"published": {"$gte": start_time, "$lte": end_time}}},
            # Only include events with IP addresses
...
class LogAnalysisService:
    ...
    def get_log_dashboard_metrics(
        ...
        # Get logs in time period (limited for performance)
        ...
        # Calculate hourly distribution for time pattern analysis
        for log in logs:
    ...
    def get_log_statistics_comparison(
        ...
        # Get severity distributions for both periods
    ...
    def _get_severity_distribution(start_time: datetime, end_time: datetime) -> Dict[str, int]:
        ...
        result = OktaEvent.objects.aggregate(*pipeline)
        return {item['_id'] if item['_id'] else 'unknown': item['count'] for item in result}
    ...
    def get_log_volume_by_timeframe(
        ...
        result = OktaEvent.objects.aggregate(*pipeline)
        return [{"timestamp": item['_id'], "count": item['count']} for item in result]